...
#import "Go2GlobalUserConfig.h"

/* callback for download string complete */
typedef void (*GGTranslationPreparedCallback)(BOOL success);

@interface Go2Global : NSObject

/**
 * Initialize Go2Global sdk
 *
 * When initializing Go2Global you must pass these three tokens. You initialize Go2Global by adding the following lines in the implementation file for your app delegate, ideally at the top of application:didFinishLaunchingWithOptions
 ...
 *
 * The code needs to correspond to the Go2Global website one to one
 ...
 * 1. If a Go2global session is already active at the time of invocation
...
+ (void)setUserConfig:(Go2GlobalUserConfig *)config;
@end
